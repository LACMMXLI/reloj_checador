...
// Agregar datos de muestra si no hay empleados
function checkAndAddSampleData() {
    const transaction = db.transaction(['employees'], 'readonly');
    const employeeStore = transaction.objectStore('employees');
    const countRequest = employeeStore.count();
...
function addSampleData() {
    const sampleEmployees = [
        { name: 'Juan Pérez', department: 'Administración', code: '123456', status: 'Activo' },
        { name: 'María López', department: 'Ventas', code: '234567', status: 'Activo' },
        { name: 'Carlos Rodríguez', department: 'Tecnología', code: '345678', status: 'Activo' },
        { name: 'Ana Martínez', department: 'Recursos Humanos', code: '456789', status: 'Activo' },
        { name: 'Roberto Sánchez', department: 'Producción', code: '567890', status: 'Inactivo' }
    ];
    ...
    sampleEmployees.forEach(employee => {
        const request = employeeStore.add(employee);
        
        request.onsuccess = (event) => {
            employeeIds.push(event.target.result);
            
            // Si es el último empleado, agregar registros de ejemplo
            if (employeeIds.length === sampleEmployees.length) {
                addSampleRecords(employeeIds, sampleEmployees);
            }
        };
    });
}
...
function addSampleRecords(employeeIds, employees) {
    const recordTransaction = db.transaction(['records'], 'readwrite');
    const recordStore = recordTransaction.objectStore('records');
    ...
    for (let i = 0; i < 7; i++) {
        ...
        // Agregar registros para cada empleado activo
        employeeIds.forEach((id, index) => {
            if (employees[index].status === 'Activo') {
                // Crear entrada y salida para cada día
                const hourIn = 8 + Math.floor(Math.random() * 2); // Entre 8 y 9
                const minuteIn = Math.floor(Math.random() * 60);
                const hourOut = 17 + Math.floor(Math.random() * 3); // Entre 17 y 19
                const minuteOut = Math.floor(Math.random() * 60);
                
                const timeIn = `${String(hourIn).padStart(2, '0')}:${String(minuteIn).padStart(2, '0')}:00`;
                const timeOut = `${String(hourOut).padStart(2, '0')}:${String(minuteOut).padStart(2, '0')}:00`;
